...
const FavoritesView: React.FC<FavoritesViewProps> = ({ favorites, surahs, onNavigate }) => {
  // Parse favorites "surahId:ayahId" to actual data objects
  const favoriteItems = favorites.map(f => {
    const [sId, aId] = f.split(':').map(Number);
    const surah = surahs.find(s => s.id === sId);
    if (!surah) return null;
    
    // Find ayah by ID (which we set as index+1 in mapping) or index
    // Since mapping sets ID to index+1, we can find index easily
  ...
  if (favoriteItems.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-8 text-light-secondary dark:text-dark-secondary">
        ...
        <h2 className="text-2xl font-semibold mb-3 text-light-text dark:text-dark-text">Henüz favori ayetiniz yok</h2>
        <p className="max-w-xs mx-auto text-sm md:text-base opacity-80">Okuma yaparken beğendiğiniz ayetlerin yanındaki kalp ikonuna tıklayarak buraya ekleyebilirsiniz.</p>
      </div>
    );
  }
